...
import { useGetBannerQuery } from "@/redux/features/bannerApi";
...
const Banner = () => {
  ...
  const {
    ...
    error: bannerError,
  } = useGetBannerQuery("status=active");
  const bannerData = banners?.data?.result;
      ...
      <div>
        <div className="relative">
          <div className="absolute top-10 md:top-0 h-full w-full text-white z-40 flex justify-center items-center ">
            <div className="max-container w-full">
              ...
              <p className="text-center text-[15px] mt-5 md:w-[70%] mx-auto">
                Find your dream home from our properties in Dhaka, Chittagong,
                Sylhet, Khulna, Rajshahi, Barishal, Rangpur, Mymensingh and
          ...
          <Drawer
            ...
            open={drawerVisible}
          >
            <div className="p-5">{/* <HomeSearchNav /> */}</div>
          </Drawer>
        </div>
      </div>

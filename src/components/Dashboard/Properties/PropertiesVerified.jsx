...
import { PROPERTY_URL, USER_URL } from "../../../../apis/url";

function PropertiesVerified({ setModalOpen, modalOPen, token, row,setUpdate, modalStyle = "" }) {
  ...
  const handelUpdate = () => {
    ...
    axios
      .patch(`${PROPERTY_URL}/${row?._id}`, body, config)
      .then((res) => {
        console.log(res);
        if (res.status === 200) {
          console.log(res);
...
export default PropertiesVerified;

...
import { blogPatch, blogPost, getOneBlogs } from "../../../../apis/blog.api";
...
function EditBlog({id}) {
  ...
  useEffect(()=>{
    if(blog){
        ...
        setValue("title", blog?.title);
        setTags(blog?.tags)
        setValueOfRichText(blog?.description)
  ...
  const onSubmit = async (values) => {
    ...
    const res = await blogPatch(body,id, token);
    console.log(res);
  ...
  return (
    <div>
      <div>
        ...
        <h2 className=" text-[25px] font-semibold">Edit Blog</h2>
        <BreakCame type={"/admin"} data={[{title:"All Blog",url:"blogs"},{title:"Edit Blog",url:""}]} />
      </div>
            ...
            <div className=" flex items-start flex-col md:flex-row gap-5 mt-5">
              ...
              <div className="flex flex-col items-start w-full ">
                ...
                <div className=" flex w-full items-center gap-2">
                  ...
                  />
                  <button type="button" onClick={()=>handelAddTag()} className="py-[15px] rounded-lg h-[50px] px-5 text-sm font-semibold bg-primary text-white">
                    Add
                  </button>
                </div>

...
function RentBuyHome({ activeBuy, setActiveBuy, data }) {
  ...
  const addFilter = () => {
    if (activeBuy) {
      setRentFilter(activeBuy === "buy" ? "sell" : activeBuy);
      if (activeBuy === "buy") {
        if (active3) {
  ...
  const content = (
    <div className="p-0 w-[275px]">
      <h2 className=" text-base mb-2 font-medium">Property Purpose</h2>
    ...
    <div className="w-full">
      ...
      >
        <button className=" bg-white capitalize border border-gray-300   w-full py-2 text-sm px-5 text-gray-700 font-normal rounded-md flex items-center justify-between">
          <span>{activeBuy ? activeBuy : "Property Purpose"}</span>
          <IoMdArrowDropdown
